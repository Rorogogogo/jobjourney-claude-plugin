...
export function registerProfileTools(server) {
    ...
    server.addTool({
        ...
        execute: async (args, context) => {
            ...
            if (args.location)
                body.location = args.location;
            if (args.skills)
                body.skills = args.skills;
            if (args.employments)
                body.employments = args.employments;
            if (args.educations)
                body.educations = args.educations;
            if (args.projects)
    ...
    server.addTool({
        ...
        execute: async (args, context) => {
            ...
            const projects = p.projects?.map(pr => `  - ${pr.name}${pr.description ? `: ${pr.description.substring(0, 80)}` : ""}${pr.url ? ` (${pr.url})` : ""}`).join("\n") || "  None listed";
            return [
