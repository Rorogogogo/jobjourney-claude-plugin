...
import { openPage, requireActivePage, closeBrowserSession, getActiveBrowser } from "./browser-session.js";
...
export function registerAutoApplyTools(server) {
    // ── open_application_page ────────────────────────────────────
    server.addTool({
        name: "open_application_page",
        description: "Open a job application page in the browser. Detects aggregator sites (LinkedIn/Seek/Indeed) and resolves to the real application URL. Keeps the browser open for subsequent form-filling tool calls.",
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Extract all visible form fields from the current page. Returns structured data for each field including label, type, options, and a CSS selector for filling. Call open_application_page first.",
        parameters: z.object({}),
        execute: async () => {
    ...
    server.addTool({
        ...
        description: "Fill a single text input or textarea field on the current page. Use the selector from extract_form_fields.",
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Select an option in a dropdown, radio group, or checkbox. Tries matching by label text first, then by value.",
        parameters: z.object({
    ...
    server.addTool({
        name: "upload_resume",
        description: "Upload a CV/resume file to a file input on the current page. Uses the primary CV by default, or specify a document ID.",
        parameters: z.object({
        ...
        execute: async (args, context) => {
            ...
            if (args.document_id) {
                ...
                if (!primary)
                    throw new Error("No CVs found. Upload a CV first.");
                if (!primary.fileUrl)
                    throw new Error("CV has no download URL.");
    ...
    server.addTool({
        name: "click_element",
        description: "Click a button or link on the current page (e.g., Next, Submit, Continue). Returns whether the page URL changed or new content appeared.",
        parameters: z.object({
    ...
    server.addTool({
        name: "set_default_cv",
        description: "Set a CV as the default/primary document for auto-apply resume uploads.",
        parameters: z.object({
    ...
    server.addTool({
        ...
        description: "Take a screenshot of the current page for visual inspection. Useful for verifying form state, handling CAPTCHAs, or debugging unusual layouts.",
        parameters: z.object({}),
        execute: async () => {
            const page = requireActivePage();
