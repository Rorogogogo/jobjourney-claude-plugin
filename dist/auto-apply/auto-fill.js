...
import { buildFillPlan } from "./fill-planner.js";
...
const MAX_PAGES = 5;
async function executePlan(page, fields, plan) {
    let filled = 0;
    const skipped = [];
    const errors = [];
        ...
        try {
            ...
            if (action.action === "fill") {
                ...
                if (tagName === "SELECT") {
                    ...
                    if (name) {
                        const radio = page.locator(`input[type=radio][name="${name}"]`);
                        const count = await radio.count();
                        let matched = false;
                        for (let i = 0; i < count; i++) {
                            const label = await radio.nth(i).evaluate((r) => {
...
async function tryUploadResume(page, fields, apiKey) {
...
function detectPageChange(urlBefore, urlAfter, fieldsBefore, fieldsAfter) {
    return urlAfter !== urlBefore || fieldsAfter !== fieldsBefore;
}
export async function autoFillApplication(page, apiKey, jobContext, onProgress) {
    ...
    for (let pageNum = 0; pageNum < MAX_PAGES; pageNum++) {
        ...
        else if (plan.confidence === "medium" && overallConfidence === "high")
        ...
        const nextButton = page.locator('button:has-text("Next"), button:has-text("Continue"), button:has-text("next"), ' +
            'a:has-text("Next"), a:has-text("Continue"), ' +
            'button[type="button"]:has-text("next step"), button:has-text("Save and continue")').first();
