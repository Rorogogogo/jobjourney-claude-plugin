 ...
 */
export async function injectAutoApplyOverlay(page) {
    await page.evaluate(() => {
        if (document.getElementById("__jj-autoapply-overlay"))
            return;
 ...
 */
export async function updateAutoApplyOverlay(page, update) {
    try {
        await page.evaluate((u) => {
            ...
            if (bar)
                bar.style.width = Math.min(100, u.percent) + "%";
 ...
 */
export async function completeAutoApplyOverlay(page, summary) {
    try {
        await page.evaluate((s) => {
            ...
            const count = document.getElementById("__jj-aa-field-count");
            if (subtitle)
                subtitle.textContent = "Auto-fill complete";
            if (spinner) {
            ...
            if (text)
                text.textContent = `Filled ${s.fieldsFilled}/${s.totalFields} fields (${s.confidence} confidence)`;
            if (bar) {
                bar.style.width = "100%";
                bar.style.background = "#16a34a";
            }
            if (count)
                count.textContent = "Please review before submitting";
