...
function formatFieldList(fields) {
    return fields
        ...
        if (f.placeholder)
            line += ` placeholder="${f.placeholder}"`;
        if (f.required)
            line += " required";
        if (f.options.length > 0)
            line += ` options=${JSON.stringify(f.options)}`;
        if (f.currentValue)
            line += ` value="${f.currentValue}"`;
        if (f.fieldGroup)
            line += ` group="${f.fieldGroup}"`;
        line += " />";
        return line;
    })
        .join("\n");
...
function formatProfile(profile) {
    ...
    if (profile.linkedinUrl)
        sections.push(`LinkedIn: ${profile.linkedinUrl}`);
    if (profile.websiteUrl)
        sections.push(`Website: ${profile.websiteUrl}`);
    if (profile.githubUrl)
        sections.push(`GitHub: ${profile.githubUrl}`);
    if (profile.workAuthorization)
        sections.push(`Work Authorization: ${profile.workAuthorization}`);
    if (profile.requiresSponsorship !== undefined)
        sections.push(`Requires Sponsorship: ${profile.requiresSponsorship ? "Yes" : "No"}`);
    if (profile.bio)
    ...
    if (addrParts.length > 0)
        sections.push(`Address: ${addrParts.join(", ")}`);
    // Salary
    ...
    if (profile.noticePeriod)
        sections.push(`Notice Period: ${profile.noticePeriod}`);
    if (profile.availableFrom)
        sections.push(`Available From: ${profile.availableFrom}`);
    // Additional
    if (profile.preferredName)
        sections.push(`Preferred Name: ${profile.preferredName}`);
    if (profile.pronouns)
        sections.push(`Pronouns: ${profile.pronouns}`);
    if (profile.nationality)
        sections.push(`Nationality: ${profile.nationality}`);
    if (profile.citizenship)
        sections.push(`Citizenship: ${profile.citizenship}`);
    if (profile.dateOfBirth)
        sections.push(`Date of Birth: ${profile.dateOfBirth}`);
    if (profile.defaultHowDidYouHear)
        sections.push(`How Did You Hear: ${profile.defaultHowDidYouHear}`);
    // EEO
    if (profile.gender)
        sections.push(`Gender: ${profile.gender}`);
    if (profile.ethnicity)
        sections.push(`Ethnicity: ${profile.ethnicity}`);
    if (profile.disabilityStatus)
        sections.push(`Disability Status: ${profile.disabilityStatus}`);
    if (profile.veteranStatus)
        sections.push(`Veteran Status: ${profile.veteranStatus}`);
    ...
    if (profile.employmentHistory?.length) {
        ...
        for (const job of profile.employmentHistory) {
            ...
            if (job.startDate)
                line += ` (${job.startDate}–${job.endDate || "present"})`;
            sections.push(line);
    ...
    if (profile.education?.length) {
        ...
        for (const edu of profile.education) {
            let line = `  - ${edu.degree}`;
            if (edu.fieldOfStudy)
                line += ` in ${edu.fieldOfStudy}`;
            line += ` at ${edu.institution}`;
            if (edu.startDate)
                line += ` (${edu.startDate}–${edu.endDate || "present"})`;
            sections.push(line);
        }
    }
    return sections.join("\n");
}
export async function buildFillPlan(fields, profile, pageContext, jobContext) {
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) {
        throw new Error("ANTHROPIC_API_KEY environment variable required for smart form filling. " +
