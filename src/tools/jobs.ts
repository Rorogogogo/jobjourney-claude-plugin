...
import { z } from "zod";
import { apiCall, API_BASE_URL, getAuthHeaders } from "../api.js";
...
export function registerJobTools(server: FastMCP<SessionAuth>) {
  server.addTool({
    ...
    execute: async (args, context) => {
      ...
      if (args.location) formData.append("Location", args.location);
      ...
      if (args.required_skills) formData.append("RequiredSkills", args.required_skills);
  ...
  server.addTool({
    ...
    execute: async (args, context) => {
      ...
      if (!eval_) return "No evaluation found for this job.";
  ...
  server.addTool({
    ...
    execute: async (args, context) => {
      ...
      if (!data.data) return "No cover letter found for this job.";
      return data.data;
    },
  });
