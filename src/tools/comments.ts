...
export function registerCommentTools(server: FastMCP<SessionAuth>) {
  ...
  server.addTool({
    ...
    execute: async (args, context) => {
      ...
      if (!thread) return "Comment thread not found.";

      const replies = thread.replies?.map((r, i) =>
        `  ${i + 1}. ${r.authorDisplayName || "Anonymous"}: ${r.content}\n     ${new Date(r.createdOnUtc).toLocaleString()}`
      ).join("\n\n") || "  No replies";
